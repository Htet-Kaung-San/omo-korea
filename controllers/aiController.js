...
if (isOpenRouterConfigured()) {
  aiResult = await generateOpenRouterMajorAnalysis(userProfile, ruleBasedRecommendations);
  method = aiResult.enabled ? 'rule-based + openrouter' : 'rule-based';
} else if (isGeminiConfigured()) {
  aiResult = await generateGeminiMajorAnalysis(userProfile, ruleBasedRecommendations);
  method = aiResult.enabled ? 'rule-based + gemini' : 'rule-based';
} else {
  aiResult = await createClaudeMajorAnalysis(userProfile, ruleBasedRecommendations);
...
const CAMPUS_FAQ = [
  {
    ...
    response: "To apply for your Alien Registration Card (ARC), you must visit the Busan Immigration Office within 90 days of your arrival. Required documents: Passport, 1 color photo (3.5x4.5cm), Certificate of Enrollment, fee (30,000 KRW), and proof of residence. You can make an appointment online at Hikorea.go.kr."
  },
  {
    keywords: ['bank', 'account', 'nh', 'card', '은행', '계좌'],
    response: "You can open a student bank account at the NH Bank (Nonghyup) branch located on the PNU campus (inside the Moonchang Hall or main administrative building). Bring your Passport, Certificate of Enrollment, and your ARC (if available)."
  },
  {
    keywords: ['insurance', 'health', 'medical', '보험', '의료', '국민건강보험'],
    response: "All international students in Korea are automatically registered for the National Health Insurance Service (NHIS) once their alien registration is processed. The monthly fee is automatically billed. For details, visit the PNU International Office."
  },
  {
    keywords: ['thesis', 'graduation', 'outline', '졸업', '논문'],
    response: "To graduate, you must submit your graduation thesis outline to your department office by the specified deadline (usually in October for the Fall semester, or April for the Spring semester). Check with your department academic advisor for outline templates."
  },
  {
    keywords: ['topik', 'korean', 'certificate', '토픽', '한국어능력시험'],
    response: "PNU graduation requirements normally specify obtaining TOPIK Level 4 or above. You must submit your official TOPIK certificate to your department office before your final semester ends."
  },
  {
    keywords: ['credit', 'audit', 'requirements', '학점'],
    response: "You must complete a credit audit to verify that your course credits meet your major requirements (usually 130+ total credits, including major required, elective, and general education). You can schedule a credit audit with your department advisor."
  },
  {
    keywords: ['cafeteria', 'food', 'eat', 'restaurant', '학식', '식당'],
    response: "Student cafeterias are located near the main library (Geumjeong Hall), Moonchang Hall, and the engineering building. Standard hours are 11:30 AM – 1:30 PM for lunch, and 5:30 PM – 7:00 PM for dinner on weekdays."
  },
  {
    keywords: ['library', 'study', 'book', '도서관'],
    response: "The PNU Central Library is open daily for student study. You can access the library and check out books using your student ID card or mobile student ID app. Study room reservations can be made via the PNU Library app."
  }
];
  ...
  try {
    ...
    if (req.body.history && Array.isArray(req.body.history)) {
      ...
      if (studentId) {
        try {
          ...
          if (error) {
            console.error('Failed to load chat history for context:', error.message);
          } else if (data) {
    ...
    if (isOpenRouterConfigured()) {
      try {
        ...
        console.error('OpenRouter Chat Error, falling back to Gemini/Claude/FAQ:', orErr.message);
      }
    }
    ...
    if (!reply && process.env.ANTHROPIC_API_KEY && process.env.CLAUDE_MODEL) {
      try {
        ...
        const response = await client.messages.create({
          ...
          max_tokens: 300,
          system: "You are the Hey! PNU Smart Assistant, an AI helper for international students at Pusan National University. Keep your responses short (under 4 sentences), friendly, helpful, and focused on PNU campus life, academics, or settlement requirements. Answer in the same language the student asks in.",
        ...
        console.error('Claude Chat Error, falling back to local FAQ:', claudeErr.message);
      }
    }
    ...
    if (!reply) {
      ...
      for (const faq of CAMPUS_FAQ) {
        const match = faq.keywords.some(keyword => lowerMsg.includes(keyword));
        if (match) {
          reply = faq.response;
          break;
        }
    ...
    if (!reply) {
      reply = "I'm the Hey! PNU Assistant. I can help you with campus inquiries such as ARC application, bank accounts, health insurance, thesis outline submissions, TOPIK requirements, library access, and cafeterias. Please ask about any of these topics!";
    }
    ...
    if (studentId) {
      try {
        ...
        if (insertError) {
          console.error('Failed to save chatbot log in database:', insertError.message);
        }
      } catch (dbErr) {
...
async function getChatHistory(req, res) {
  try {
    ...
    if (error) {
      console.error('Error fetching chat history from Supabase:', error.message);
...
async function clearChatHistory(req, res) {
  try {
    ...
    if (error) {
      console.error('Error clearing chat history from Supabase:', error.message);
