...
if (isPlaceholder) {
  console.log(
    "--- SUPABASE DETECTED AS PLACEHOLDER: USING LOCAL JSON DATABASE FALLBACK ---",
  );
        ...
        promise.then(({ data, error }) => {
          ...
          if (records.length === 0) {
            ...
            if (
              ...
              context.selectStr &&
              context.selectStr.includes("major")
            ) {
              const major = localDb.findOne(
                "majors",
                (m) => m.major_id === record.major_id,
              );
              record = { ...record, major };
            }
      ...
      return makePromise((resolve) => {
        promise.then(({ data, error }) => {
          ...
          const sorted = [...records].sort((a, b) => {
            if (a[field] < b[field])
              return opts && opts.ascending === false ? 1 : -1;
            if (a[field] > b[field])
              return opts && opts.ascending === false ? -1 : 1;
            return 0;
          });
          ...
          return makePromise(
            (resolve) => {
              ...
              if (table === "post") {
                ...
                records = records.map((record) => {
                  const course = localDb.findOne(
                    "courses",
                    (c) => Number(c.course_id) === Number(record.course_id),
                  );
