...
import { api } from '@/api'
import type { ChecklistItem, ChecklistVariant, GraduationProgress } from '@/types/api'
...
export function ChecklistPage() {
  ...
  useEffect(() => {
    Promise.all([api.getChecklist(), api.getGraduationProgress()])
      ...
      .catch((err) => setError(err instanceof Error ? err.message : t('checklist.loadError')))
  ...
  return (
    <div>
      <PageHeader
        title={isFreshmanChecklist ? t('checklist.titleFreshman') : t('checklist.titleGraduation')}
        subtitle={
          isFreshmanChecklist
      ...
      <div className="space-y-4 px-5 py-5">
          ...
          <div className="rounded-2xl border border-pnu-border bg-white p-4">
            <div className="mb-2 flex items-center justify-between text-sm">
              ...
              <span className="text-pnu-muted">
                {t('common.completedCount', { completed: completedCount, total: items.length })}
              </span>
            </div>
        ...
        {!isFreshmanChecklist && progress && !loading && (
          <div className="rounded-2xl border border-dashed border-pnu-border bg-white/70 px-4 py-3 text-[12px] text-pnu-muted leading-relaxed">
            <p className="font-semibold text-pnu-text mb-1">
              ...
              <span className="text-pnu-blue-light">{progress.totalCompleted}</span>
              <span className="text-pnu-muted"> / {progress.totalRequired}</span>
            </p>
            <p>
              {t('checklist.lockLegend')}
            </p>
          </div>
        )}
        ...
        {loading ? <p className="text-sm text-pnu-muted">{t('checklist.loading')}</p> : null}

        <div className="space-y-3">
          {items.map((item) => {
            const locked = isItemLocked(item, progress)
            const lockReason = locked ? getLockReason(item, progress, t) : undefined
            return (
              <ChecklistRow
                key={item.id}
                item={item}
