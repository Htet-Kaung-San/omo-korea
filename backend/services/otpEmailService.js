 ...
 */
/**
 * Shared transport. Everything user-facing that leaves this backend goes
 * through here, so the test guard, the timeout and the error shape are
 * identical for every message type.
 *
 * @throws {Error} code OTP_DELIVERY_FAILED when the message was not accepted.
 */
async function deliver({ to, subject, html, text }) {
  // Fail closed under Jest. No suite reaches this today — they all use the demo
  // account, LOGIN_OTP_FIXED, or a mock — but a future test written with an
  // ordinary address would otherwise send real mail from CI against a
  // rate-limited provider. Mock this module instead.
  if (process.env.NODE_ENV === "test" && process.env.ALLOW_REAL_OTP_EMAIL !== "1") {
    const error = new Error(
      "Refusing to send real email from the test environment. Mock services/otpEmailService instead.",
    );
...
function buildPasswordResetEmail(actionLink) {
  // The link is Supabase's — generated by auth.admin.generateLink and carrying
  // its own single-use token — so it is placed verbatim, never rewritten.
  const safeLink = escapeHtml(actionLink);
  return {
    subject: "Reset your Hey! PNU password",
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:480px;margin:0 auto;padding:24px">
        <h1 style="font-size:20px;color:#003D82;margin:0 0 8px">Hey! PNU</h1>
        <p style="font-size:15px;color:#334155;margin:0 0 20px">
        ...
        <p style="margin:0 0 20px">
          <a href="${safeLink}" style="display:inline-block;background:#003D82;color:#fff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 22px;border-radius:12px">
            Reset password
          </a>
        </p>
        <p style="font-size:12px;color:#64748B;margin:0 0 16px;word-break:break-all">
          Or paste this into your browser:<br>${safeLink}
        </p>
        <p style="font-size:13px;color:#64748B;margin:0">
    ...
    `.trim(),
    text: `Hey! PNU\n\nUse this link to choose a new password:\n${actionLink}\n\nThe link can only be used once. If you did not ask to reset your password, you can ignore this email — nothing has changed.`,
  };
}

async function sendPasswordResetEmail({ to, actionLink }) {
  const { subject, html, text } = buildPasswordResetEmail(actionLink);
  return deliver({ to, subject, html, text });
}
