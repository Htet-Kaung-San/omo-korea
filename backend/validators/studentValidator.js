...
const { SUPPORTED_LANGUAGE_PREFS } = require('../middleware/supportedLanguages');
...
const updateProfileSchema = Joi.object({
  ...
  language_pref: Joi.string()
    ...
    .custom((value, helpers) => {
      const { resolveLanguagePref } = require('../middleware/supportedLanguages');
      const resolved = resolveLanguagePref(value);
      if (!resolved) {
    ...
    .messages({
      'any.invalid': `language_pref must be one of: ${SUPPORTED_LANGUAGE_PREFS.join(', ')}`,
    }),
