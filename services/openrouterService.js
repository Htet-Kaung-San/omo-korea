...
async function generateOpenRouterChat(message, history = []) {
  ...
  if (history && history.length > 0) {
    ...
    messagesPayload.push({ role: 'user', content: `${systemInstruction}\n\nUser Question: ${message}` });
  }
  ...
  for (const { id: model, timeout } of modelConfig) {
    try {
      console.log(`Attempting OpenRouter chat with model: ${model} (timeout: ${timeout}ms)`);
      ...
      if (data.error) {
        throw new Error(data.error.message || 'OpenRouter returned error object');
      }
...
Return valid JSON ONLY matching this format (no markdown blocks, no prefix/suffix):
{
  ...
  for (const { id: model, timeout } of modelConfig) {
    try {
      console.log(`Attempting OpenRouter major analysis with model: ${model} (timeout: ${timeout}ms)`);
      ...
      console.log(`Successfully generated major analysis using model: ${model}`);
      return {
        enabled: true,
        analysis: parsedJSON,
        warning: null
      ...
      console.error(`OpenRouter major recommendation failed with model ${model}:`, error.message);
      lastError = error;
      // Fallback to next model
    }
  }
