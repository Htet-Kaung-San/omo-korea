...
let englishOneStopPromise: Promise<MessageDictionary> | null = null

async function loadEnglishOneStop(): Promise<MessageDictionary> {
  if (!englishOneStopPromise) {
    englishOneStopPromise = LOCALE_LOADERS.en().then((module) => module.default)
  }
  return englishOneStopPromise
}

export async function loadOneStopMessages(language: LanguageCode): Promise<MessageDictionary> {
