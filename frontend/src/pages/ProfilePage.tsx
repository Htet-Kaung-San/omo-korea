...
export function ProfilePage() {
  ...
  const [languagePref, setLanguagePref] = useState<LanguageCode>(language)
  ...
  return (
    <div>
      ...
      <form className="space-y-5 px-5 py-5" onSubmit={handleSave}>
        ...
        <div className="space-y-1.5">
          <label htmlFor="languagePref" className="block text-sm font-medium text-pnu-text">
            {t('profile.language') !== 'profile.language' ? t('profile.language') : 'Preferred language'}
          </label>
          <select
            id="languagePref"
            value={languagePref}
            onChange={(e) => setLanguagePref(e.target.value as LanguageCode)}
            className="w-full rounded-xl border border-pnu-border bg-white px-3.5 py-3 text-sm outline-none focus:border-pnu-blue-light focus:ring-2 focus:ring-pnu-blue-light/20"
          >
            {options.map((option) => (
